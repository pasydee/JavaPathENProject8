...
public class User {
    ...
    private final Set<UUID> rewardedAttractionIds = ConcurrentHashMap.newKeySet();
    ...
    public Date getLatestLocationTimestamp() { return latestLocationTimestamp; }

    public void addToVisitedLocations(VisitedLocation visitedLocation) {
        visitedLocations.add(visitedLocation);
    }

    public List<VisitedLocation> getVisitedLocations() { return visitedLocations; }

    public void clearVisitedLocations() { visitedLocations.clear(); }

    public void addUserReward(UserReward userReward) {

        UUID attractionId = userReward.attraction.attractionId;

        // Vérification instantanée
        if (rewardedAttractionIds.contains(attractionId)) return;
    ...
    public VisitedLocation getLastVisitedLocation() {
        if (visitedLocations.isEmpty()) return null;
        return visitedLocations.get(visitedLocations.size() - 1);
    }
    public Set<UUID> getRewardedAttractionIds() {
        return rewardedAttractionIds;
    }

    public void setTripDeals(List<Provider> tripDeals) { this.tripDeals = tripDeals; }
